<script lang="ts">
  ...
  const years = Array.from({ length: 80 }, (_, index) => {
	...
	let selectedMonth = currentMonth.getMonth();
	...
	{#if !inline}
		<div class="relative">
			<input
				...
				type="text"
				class="w-full px-4 py-2 text-sm border rounded-md focus:outline-hidden dark:bg-gray-700 dark:text-white dark:border-gray-600 {getFocusRingClass(
					color
				)} {inputClass}"
				{placeholder}
			...
			<button
				type="button"
				class="absolute inset-y-0 right-0 flex items-center px-3 text-gray-500 dark:text-gray-400 focus:outline-hidden"
