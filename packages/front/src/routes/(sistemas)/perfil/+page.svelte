<script lang="ts">
	...
	import { UserCircleOutline, EnvelopeOutline, ShieldCheckOutline } from 'flowbite-svelte-icons';
		...
		<div class="max-w-4xl mx-auto space-y-8">
			...
			<div class="flex flex-col items-center md:flex-row md:items-start gap-12">
				...
				<div class="flex flex-col items-center gap-6">
					<div class="relative">
						<Avatar src={`https://unavatar.io/${usuario?.correo}`} size="xl" class="w-40 h-40 ring-4 ring-primary-500 ring-offset-4 ring-offset-white" />
						<div class="absolute -bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 bg-primary-500 text-white text-sm font-medium rounded-full">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div class="p-4 bg-white rounded-lg border border-gray-200">
							...
							<div class="space-y-2 ml-11">
								...
								<p class="text-gray-900 font-medium">Pregunta de seguridad: {usuario?.pregunta_configurada ? 'Configurada' : 'Pendiente'}</p>
								<p class="text-gray-900 font-medium">Contraseña: {usuario?.cambiar_clave ? 'Debe cambiar su contraseña' : 'Actualizada'}</p>
							</div>
						</div>
					</div>
				</div>
			...
			<Card class="p-6 shadow-lg">
				...
				<form method="POST" action="?/cambiarPassword" use:enhance>
					<div class="space-y-4">
						<div>
						...
						<Button type="submit" class="w-full" disabled={!isPasswordValid}>
							Cambiar Contraseña
						</Button>
					</div>
				</form>
			...
			<Card class="p-6 shadow-lg">
				...
				<form method="POST" action="?/configurarPregunta" use:enhance>
					<div class="space-y-4">
						<div>
						...
						<Button type="submit" class="w-full" disabled={!canSubmitQuestion}>
							Configurar Pregunta
						</Button>
					</div>
				</form>
