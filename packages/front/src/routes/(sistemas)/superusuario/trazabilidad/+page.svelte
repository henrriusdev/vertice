...
<div class="container mx-auto px-4 py-8">
    ...
    <form
            ...
            use:enhance={() => {
              exportando = true;
              return resolver(()=>{exportando=false});
            }}
            style="display: none;"
    >
            ...
            <div class="p-4 flex justify-between items-center flex-wrap gap-4">
                <div>
                    <p class="text-sm text-gray-700">
                        Mostrando <span class="font-medium">{(paginaActual - 1) * elementosPorPagina + 1}</span> a <span
                            class="font-medium">{Math.min(paginaActual * elementosPorPagina, data.registros.length)}</span>
